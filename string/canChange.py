...
class Solution:
    def _compareMaps(self, startMap, targetMap):
    ...
    def _getMap(self, letters):
        lettersMap = []
        for i in range(len(letters)):
            letter = letters[i]
            if letter in ["L", "R"]:
                lettersMap.append(Piece(letter, i))

        return lettersMap

    def canChange(self, start: str, target: str) -> bool:
        startMap = self._getMap(start)
        targetMap = self._getMap(target)

        if len(startMap) != len(targetMap):
            return False
